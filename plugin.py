...
from enigma import eDVBDB, eEPGCache, eTimer, quitMainloop
...
config.plugins.StayUP.DebugMode = ConfigYesNo(default=False)
config.plugins.StayUP.AutoUpdate = ConfigYesNo(default=False)
config.plugins.StayUP.RemovePlugin = ConfigYesNo(default=False)
...
class StayUPBackgroundWorker():
	...
	def ExecTask(self):
		writeLog("030 Timer Execution Called")
		self.loop.stop()
		if config.plugins.StayUP.Enabled.value:
			writeLog("035 Looking for new files...")
			if not config.plugins.StayUP.RemovePlugin.value:
				download(self)
			else:
				witeLog("031 Plugin has to be removed running disabled")
				config.plugins.StayUP.Enabled.value = False
...
class StayUPConfigFunction(ConfigListScreen, Screen):
	skin = """
		<screen position="80,170" size="560,270" title="StayUP vr10.090209">
	...
	def __init__(self, session):
		...
		list = []
                ...
                list.append(getConfigListEntry(_("Remove Plugin(press blue key):"), config.plugins.StayUP.RemovePlugin))
	...
	def about(self):
		...
		self.session.open(MessageBox, (_("StayUP - For lazy people (by Claudio72)\nDreambox autoupdater.\nSpecial thanks goes to the EDG - Nemesis crew for the support\n\nThanks to marcus206(settings maintainer)\nThanks to vHannibal for the legendary settings")), MessageBox.TYPE_INFO)
	def execute(self):
		writeLog("052 Blue Key Pressed")
		if config.plugins.StayUP.Epg.value:
			if not os.path.exists("/usr/bin/nemesisd"):
				self.session.open(MessageBox,("Auto EPG really works only on a pure Edg Nemesis IMAGE"), MessageBox.TYPE_INFO)
				config.plugins.StayUP.Epg.value = False
                if not config.plugins.StayUP.RemovePlugin.value:
			download(self)
                else:
                        writeLog("053 Removing Plugin")
                        os.system ("rm /usr/log/*.SIGNATURE")
                        os.system ("rm /usr/lib/enigma2/python/Plugins/Extensions/StayUP/*")
                        os.system ("rmdir /usr/lib/enigma2/python/Plugins/Extensions/StayUP")
			config.plugins.StayUP.RemovePlugin.value = False
			config.plugins.StayUP.Enabled.value = False
			for x in self["config"].list:
				x[1].save()
			self.session.openWithCallback(self.DoChangeTimeRestart,MessageBox,_("Enigma2 will restart now - OK ?"), MessageBox.TYPE_YESNO)

	def logviewer(self):
		writeLog("054 Yellow Key Pressed")
		self.session.open(Console, "stayUP", ["cat /tmp/StayUpPlugin.log"])

	def save(self):
		writeLog("056 Red Key Pressed")
		if config.plugins.StayUP.Epg.value:
			if not os.path.exists("/usr/bin/nemesisd"):
				self.session.open(MessageBox,("Auto EPG really works only on a pure Edg Nemesis IMAGE"), MessageBox.TYPE_INFO)
				config.plugins.StayUP.Epg.value = False
	...
	def DoChangeTimeRestart(self,answer):
        	if answer is None:
        		writeLog("098 Not Answered")
        	if answer is False:
        		writeLog("099 Not Confirmed")
        	else:
        		writeLog("100 Enigma2 Restart, see you again")
        		quitMainloop(3)
