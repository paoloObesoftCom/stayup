...
def download(self):
	...
	self.Alive="Alive.txt"
#new
	RetCode=os.system ("/usr/lib/enigma2/python/Plugins/Extensions/StayUP/ping_host.sh")
	if not RetCode == 0:
		writeLog("101 Network not ready or Server not responding")
		return
	RetCode=os.system ("wget -O /tmp/" + self.Alive + " " + self.location_download + self.Alive)
	if not RetCode == 0:
		writeLog("102 Download temporary disabled")
		return
